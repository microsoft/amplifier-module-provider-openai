"""Tests for `tool_search.mode: off | namespaced` (OpenAI-only).
...
def test_default_config_tools_sha256_is_pinned():
    ...
    expected = hashlib.sha256(_canon(_expected_off_mode_tools()).encode()).hexdigest()
...
def test_explicit_off_matches_the_default():
    default = _run(_make_provider(), _request())["tools"]
    explicit = _run(_make_provider(tool_search={"mode": "off"}), _request())["tools"]
    assert _canon(default) == _canon(explicit)


def test_unknown_mode_fails_loud_at_construction():
    with pytest.raises(ValueError, match="tool_search.mode"):
        _make_provider(tool_search={"mode": "deferred"})


def test_flat_mode_is_not_offered():
    """`flat` is refused on purpose -- 1,270 tok vs 7,573 for the namespace form."""
    with pytest.raises(ValueError, match="tool_search.mode"):
        _make_provider(tool_search={"mode": "flat"})


def test_non_mapping_tool_search_config_fails_loud():
    with pytest.raises(ValueError, match="tool_search must be a mapping"):
        _make_provider(tool_search="namespaced")
...
def _deferred_tools() -> list[dict[str, Any]]:
    return _run(_make_provider(tool_search={"mode": "namespaced"}), _request())["tools"]
...
def test_member_definitions_are_unchanged_apart_from_defer_loading():
    tools = _deferred_tools()
    static_by_name = {t["name"]: t for t in _expected_off_mode_tools()}
    for t in tools:
        if t.get("type") != "namespace":
            continue
        for member in t["tools"]:
...
def test_always_loaded_is_configurable():
    provider = _make_provider(
        tool_search={"mode": "namespaced", "always_loaded": ["bash"]}
    )
...
def test_off_mode_does_not_force_tool_choice():
    provider = _make_provider()
    provider.client.responses.create = AsyncMock(return_value=DummyResponse())
    asyncio.run(provider.complete(_request(), tool_choice="required"))
...
def test_off_mode_never_emits_additional_tools():
    provider = _make_provider()
...
# ---------------------------------------------------------------------------
# 4. BREAK 6 -- NULL call_id AND THE NAMESPACED function_call
#
# The break-3 round trip (capture + re-emission) lives in its own file,
# tests/test_break3_tool_search_round_trip.py, because it is the pre-registered
# FAIL-BEFORE guard and must import nothing this feature adds.
# ---------------------------------------------------------------------------
...
def test_bad_table_fails_at_provider_construction():
    with pytest.raises(ValueError):
        _make_provider(
            tool_search={
                "mode": "namespaced",
                "namespaces": [{"name": "web", "members": ["web_fetch"]}],
            }
        )
