...
- `model` – the model slug (e.g., `gpt-5.4`, `o4-mini`).
...
- The `reasoning` object lets you hint at reasoning effort levels and summary verbosity (`concise`, `detailed`, etc.).
- **`reasoning.effort`** — the accepted set is model-specific. Verified live against `gpt-5.6-sol` (2026-07-14): `none`, `low`, `medium`, `high`, `xhigh`, `max`. Note `minimal` is **not** accepted by gpt-5.6-sol (`400`), and `max` is accepted even though the API's own invalid-value error text does not yet list it. `max` is the most token-hungry level.
- **`reasoning.mode`** (GPT-5.6) — `standard` (default) or `pro`. `pro` runs deeper internal reasoning and returns a single final answer. Verified live 2026-07-14 (`{"reasoning": {"mode": "pro"}}` → `200`, echoed back). Sending `mode` to a model that does not support it returns `400`, so the provider only forwards it when the caller sets it.
- Every reasoning echo also carries `reasoning.context` (`auto` | `all_turns` | `current_turn`) for persisted reasoning; the provider does not set it today (server default `all_turns` was observed).
...
- `prompt_cache_retention` (`"in_memory"` | `"24h"` | omitted) hints at the cache TTL. The provider defaults this to `"24h"` so non-gpt-5.5 models get extended GPU-local KV storage instead of the much-shorter per-model in_memory default. The capability layer (`supports_in_memory_retention`, `supports_24h_retention`) auto-drops values a model would reject. **gpt-5.6** rejects `"in_memory"` (`400`: "This model is compatible only with 24h extended prompt caching"), so its capability descriptor sets `supports_in_memory_retention=False`.
- `prompt_cache_options` (GPT-5.6) is an explicit prompt-cache control object: `{"mode": "implicit" | "explicit", "ttl": "30m"}`. **It COEXISTS with `prompt_cache_retention` — it does NOT replace it** (verified live 2026-07-14: both are echoed back together). `mode` accepts `implicit` or `explicit`; `ttl` currently accepts only `"30m"` (the API rejects other values, so the provider forwards `ttl` as-is and lets the API validate it). The provider only sends `prompt_cache_options` when explicitly configured/passed.
