...
# Config values for the `tool_search.mode` knob (DESIGN.md w3 §2.2a).
TOOL_SEARCH_MODE_OFF = "off"
TOOL_SEARCH_MODE_NAMESPACED = "namespaced"
TOOL_SEARCH_MODES = frozenset({TOOL_SEARCH_MODE_OFF, TOOL_SEARCH_MODE_NAMESPACED})
...
class ToolSearchConfigError(ValueError):
    """Raised at mount time for an unusable `tool_search` configuration.
...
def validate_tool_search_mode(mode: Any) -> str:
    """Normalise and validate the `tool_search.mode` config value."""
    if mode is None or mode == "":
        return TOOL_SEARCH_MODE_OFF
    if not isinstance(mode, str) or mode not in TOOL_SEARCH_MODES:
        raise ToolSearchConfigError(
            f"tool_search.mode must be one of {sorted(TOOL_SEARCH_MODES)}; "
...
def normalize_namespaces(raw: Any) -> tuple[dict[str, Any], ...]:
    ...
    if not isinstance(raw, list) or not raw:
        raise ToolSearchConfigError(
            "tool_search.namespaces must be a non-empty list of "
    ...
    for entry in raw:
        if not isinstance(entry, dict):
            raise ToolSearchConfigError(
                "tool_search.namespaces entries must be mappings; got "
                f"{type(entry).__name__}."
            )
        name = entry.get("name")
        if not isinstance(name, str) or not name:
            raise ToolSearchConfigError(
                "each tool_search.namespaces entry needs a 'name'."
            )
